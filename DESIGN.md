...
Clay의 웹사이트는 B2B 데이터 인리치먼트를 딱딱한 엔터프라이즈 업무가 아니라 손맛이 느껴지는 작업처럼 다루는, 따뜻하고 장난기 있는 색채 중심의 디자인입니다. 이 디자인 언어는 따뜻한 크림색 배경(`#faf9f7`)과 오트 톤의 보더(`#dad4c8`, `#eee9df`)를 바탕으로 하며, 모든 표면에 수제 종이 같은 촉감을 부여합니다. 이 장인적인 캔버스 위에는 Matcha 그린, Slushie 시안, Lemon 골드, Ube 퍼플, Pomegranate 핑크, Blueberry 네이비, Dragonfruit 마젠타 같은 개성 강한 스와치 팔레트가 터져 나오며, 각각의 이름도 일반적인 엔터프라이즈 UI 키트의 색상명이 아니라 주스 바의 메뉴처럼 느껴집니다.
...
**핵심 특징:**
- 따뜻한 크림 캔버스(`#faf9f7`)와 오트 톤 보더(`#dad4c8`) 조합. 임상적이기보다 공예적이어야 함
...
- **Pure White** (`#ffffff`): 카드 배경, 버튼 배경, 반전 텍스트
- **Warm Cream** (`#faf9f7`): 페이지 배경. 따뜻하고 종이 같은 캔버스
...
### 여백 철학
- **따뜻하고 넉넉한 호흡**: 크림 배경은 콘텐츠 블록 사이에 따뜻한 쉼을 만든다. 여백은 넉넉하지만 차갑게 비어 있지는 않으며, 잘 차려진 테이블처럼 초대하는 감각이 있어야 한다.
...
| 레벨 | 처리 방식 | 사용처 |
|------|-----------|--------|
| Flat (Level 0) | 그림자 없음, 크림 캔버스 | 페이지 배경 |
...
### Do
- 페이지 배경은 반드시 따뜻한 크림(`#faf9f7`)을 사용한다. 이 따뜻함이 정체성이다.
...
### Don't
- 차가운 회색 배경을 쓰지 않는다. 따뜻한 크림(`#faf9f7`)은 타협 불가다.
...
### 예시 컴포넌트 프롬프트
- "Warm cream (`#faf9f7`) 배경 위에 hero를 만들어라. 헤드라인은 80px Roobert weight 600, line-height 1.00, letter-spacing -3.2px, OpenType 'ss01 ss03 ss10 ss11 ss12', 검은 텍스트. 서브타이틀은 20px weight 400, line-height 1.40, `#9f9b93` 텍스트. 버튼은 두 개: white solid pill(12px radius) 하나와 ghost outlined(4px radius, `1px solid #717989`) 하나."
...
### 반복 작업 가이드
1. 시작점은 항상 warm cream(`#faf9f7`)이다. 차가운 흰색으로 시작하지 않는다.
